...
def draw_landmarks_on_image(rgb_image, detection_result):
    ...
    for idx in range(len(face_landmarks_list)):
        ...
        global points
        points = np.array([[face_landmarks[i].x, face_landmarks[i].y, face_landmarks[i].z] for i in facemarkers.FACEMESH_EYES], dtype="float32")
        ...
        solutions.drawing_utils.draw_landmarks(
            ...
            connections=facemarkers.FACEMESH_LEFT_EYE,
            landmark_drawing_spec=None)

        solutions.drawing_utils.draw_landmarks(
            image=annotated_image,
            landmark_list=face_landmarks_proto,
            connections=facemarkers.FACEMESH_RIGHT_EYE,
            landmark_drawing_spec=None)

        solutions.drawing_utils.draw_landmarks(
            image=annotated_image,
            landmark_list=face_landmarks_proto,
            connections=facemarkers.FACEMESH_LEFT_IRIS,
            landmark_drawing_spec=None)

        solutions.drawing_utils.draw_landmarks(
            image=annotated_image,
            landmark_list=face_landmarks_proto,
            connections=facemarkers.FACEMESH_RIGHT_IRIS,
            landmark_drawing_spec=None)
