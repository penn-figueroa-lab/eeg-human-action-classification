...
from recording_nodes import WEBCAMSUB, EEGSUB, LABELPUB, OPTITRACKSUB

root = tk.Tk()
w0, h0 = 1920, 1080  # monitor 1
w1, h1 = 1920, 1080  # monitor 2

dir = '/home/choi/catkin_ws/src/eeg_data_recording/log/'
file_name = ''
...
def recording():
    ...
    command = "rosbag record -o " + file_name + " /eye_publisher /eeg_publisher /natnet_ros/fullbody/pose"
    rosbag_proc = subprocess.Popen(command, stdin=subprocess.PIPE, shell=True, cwd=dir)
    time.sleep(1)

    with open(dir + file_name + '.csv', 'w') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(neurone.label + webcam.label + optitrack.label + ['cue'] + ['year', 'day', 'hour', 'minute', 'second', 'microsecond'])
        ...
        for i, (cue, interval) in enumerate(cues):
            ...
            while time.time() - time_start < interval:
                label_pub.pub.publish(cue_states[cue])
                ...
                csvwriter.writerow(neurone.data + webcam.data + optitrack.data + [cue_states[cue]] + time_list)
